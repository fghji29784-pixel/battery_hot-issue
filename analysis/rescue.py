...
def main(spec, at=None, rows=None, cols=None, order="col"):
    ...
    # [2-b] 는 원리로 예상하고, 여기는 측정한다. 둘이 어긋나면 측정이 이긴다.
    # 어긋난 채로 두면 위의 권고가 아래 표를 부정하는 글이 되어 읽는 사람이
    # 어느 쪽을 믿어야 할지 모른다. 그래서 명시적으로 대조한다.
    if worst and rec_2b and rec_2b in worst:
        best = min(worst, key=worst.get)
        r_, b_ = worst[rec_2b], worst[best]
        # 차이가 작으면 '빗나갔다' 고 할 게 아니다. 불량이 4~8개뿐이라
        # 최악셀 검사율 몇 %p 차이는 셀 한 개 순위로도 뒤집힌다.
        gap_ok = (r_ - b_) >= 0.02 and (b_ <= 0 or r_ / max(b_, 1e-12) >= 1.5)
        if best == rec_2b:
            print(f"\n    ★ [2-b] 의 예상과 실측이 일치한다 — '{best}' 가 최선이다.")
        elif not gap_ok:
            print(f"""
    · [2-b] 가 꼽은 '{rec_2b}'({r_*100:.2f}%) 와 실제 최선
      '{best}'({b_*100:.2f}%) 의 차이가 작다. 불량이 {int(y.sum())}개뿐이라
      이 정도 차이는 셀 한 개 순위로 뒤집힌다. 둘 중 하나로 단정하지 말 것.""")
        else:
            fold = r_ / max(b_, 1e-12)
            print(f"""
    ★ [2-b] 의 예상이 빗나갔다 — 측정이 이긴다
      [2-b] 는 원리상 '{rec_2b}' 가 맞다고 봤다.   실측 {r_*100:.2f}%
      그런데 실제로 가장 낮은 것은 '{best}' 다.    실측 {b_*100:.2f}%
      {fold:.0f}배 차이다. 오차로 볼 수 있는 폭이 아니다.
      → 원리는 '모양이 트레이마다 같으니 크기만 맞추면 된다' 였다.
        실제로는 공통 모양이 못 따라가는 트레이별 굴곡이 있고,
        그것까지 빼는 쪽이 검출에서 이겼다는 뜻이다.
      ※ [2-b] 의 권고가 아니라 이 표를 따를 것. [5] 의 시점 안정성으로
        한 번 더 확인할 것 — 우연히 한 시점만 좋은 것일 수 있다.""")
